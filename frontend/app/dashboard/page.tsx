...
function LeadRow({ lead }: { lead: Lead }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 20px", borderBottom: "1px solid #f0f0f2" }}>
      <div style={{
        width: 34, height: 34, borderRadius: "50%", background: "#F4F0FF",
        border: "1.5px solid #E4DEFF", display: "flex", alignItems: "center",
        justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#6E56CF", flexShrink: 0,
      }}>
        {initials}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        ...
        <Link href={`/outreach?ids=${lead.id}`}
          style={{ fontSize: 11, color: "#6E56CF", textDecoration: "none", fontWeight: 600, background: "#F4F0FF", padding: "4px 10px", borderRadius: 5 }}>
          Message →
        </Link>
      </div>
    </div>
  ...
  return (
    <>
      <div className="page-header">
        ...
        <Link href="/workspace" className="btn-primary">Explore campaigns</Link>
        <Link href="/prospecting" className="btn-secondary">
      ...
      <div style={{ padding: "0 40px 40px" }}>
        ...
        {/* Send cap mini-bar */}
        {sendCap && (
          <div style={{ marginBottom: 20, padding: "10px 16px", background: sendCap.is_capped ? "#fff1f2" : "#f7f7f8", border: `1px solid ${sendCap.is_capped ? "#E4DEFF" : "#e8e8ea"}`, borderRadius: 9, display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 100, height: 5, background: "#e8e8ea", borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${Math.min(100, sendCap.pct_used)}%`, background: sendCap.is_capped ? "#6E56CF" : sendCap.pct_used >= 75 ? "#f59e0b" : "#22c55e", borderRadius: 3 }} />
              </div>
              <Link href="/analytics" style={{ fontSize: 11, color: "#6E56CF", fontWeight: 600, textDecoration: "none" }}>Analytics →</Link>
            </div>
          </div>
        )}
        ...
        {hasLeads ? (
          ...
          <div className="card" style={{ overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: "1px solid #e8e8ea" }}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                ...
                <Link href="/leads" style={{ fontSize: 12, color: "#6E56CF", textDecoration: "none", fontWeight: 600 }}>
                  All leads →
                </Link>
              </div>
            </div>
          ...
          <div className="card" style={{ overflow: "hidden" }}>
            <div style={{ padding: "56px 40px", textAlign: "center" }}>
              <div style={{ width: 52, height: 52, borderRadius: 14, background: "#F4F0FF", border: "2px solid #E4DEFF", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
                <svg viewBox="0 0 24 24" fill="#6E56CF" width={24} height={24}>
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              </div>
                  ...
                  <div key={s.step} style={{ display: "flex", alignItems: "stretch" }}>
                    <div style={{ width: 220, padding: "20px 20px", textAlign: "left" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                        <span style={{ width: 22, height: 22, borderRadius: "50%", background: "#6E56CF", color: "#fff", fontSize: 11, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          {s.step}
                        </span>
                      ...
                      <Link href={s.href} style={{ fontSize: 12, color: "#6E56CF", fontWeight: 600, textDecoration: "none" }}>{s.cta} →</Link>
                    </div>
                    {i < 2 && (
