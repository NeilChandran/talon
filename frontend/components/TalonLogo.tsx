/** Talon brand — classic bird mark (white) + wordmark */
...
export function TalonMark({ size = 28, className }: { size?: number; className?: string }) {
  return (
    <div
      className={`talon-mark-anim ${className ?? ""}`.trim()}
    ...
    >
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none">
        ...
        <g transform="translate(4, 4)">
          <path fill="#FFFFFF" d={TWITTER_BIRD} />
        </g>
      </svg>
    </div>
  );
...
export function TalonWordmark({
  ...
  return (
    <span
      className={`talon-wordmark-text ${className ?? ""}`.trim()}
      style={{ fontSize: height * 1.35, lineHeight: 1 }}
      aria-label="Talon"
    >
      Talon
    </span>
